 ...
 * - 버튼: "무기만 (아무거나)"
 * - 능력치 균등 그리드 + 각 능력 밑에 보너스 배지(+2/+1)
 * - 기술: 배경 고정 2개는 제외하고 "내가 찍을" 클래스 선택 N개만 표시
 * - 재주: 한글만 (영문 병기 제거)
 * - 언어 전환: EN 모드에서 Race/Class/Background/Weapons/Skills를 영어로 표시
 * - Dice Roller: 합계 제거(개별 주사위만 표시)
 * - 승자 정하기: 각 플레이어 굴림 값 표시 + 승자 표시
 */
...
// 클래스별 선택 가능 스킬과 개수 (배경 2개는 제외하고 N개 추천)
...
const SIMPLE_KO: Record<(typeof SIMPLE)[number], string> = {
  "Club":"곤봉","Dagger":"단검","Greatclub":"대형 곤봉","Handaxe":"손도끼","Javelin":"투창","Light Crossbow":"경쇠뇌","Light Hammer":"경량 망치","Mace":"철퇴","Quarterstaff":"육척봉","Shortbow":"단궁","Sickle":"낫","Spear":"창"
};
...
const MARTIAL_KO: Record<(typeof MARTIAL)[number], string> = {
  "Battleaxe":"전투 도끼","Flail":"도리깨","Scimitar":"협도","Greataxe":"대형 도끼","Greatsword":"대검","Halberd":"미늘창","Hand Crossbow":"손 쇠뇌","Heavy Crossbow":"중쇠뇌","Longbow":"장궁","Longsword":"장검","Maul":"대형 망치","Morningstar":"모닝스타","Pike":"장창","Rapier":"레이피어","Glaive":"언월도","Shortsword":"소검","Trident":"삼지창","Warhammer":"전쟁 망치","War Pick":"전쟁 곡괭이"
};
...
export default function App(){
  ...
  const [weaponsKO, setWeaponsKO] = useState<string[]>([]); // 내부 계산은 KO
  ...
  function rollWeapons(){
    ...
    if (pool.length === 0) { setWeaponsKO( randomAny2KO() ); return; }
  ...
  function rollSkills(){
    ...
    setSkills(shuffle(pool).slice(0, cfg.n)); // 배경 2개 제외 → 내가 찍을 것만
  }
  function rollFeat(){ 
    // 한글만
    const feats = ["능력 향상","배우","경계","운동선수","돌격자","쇠뇌 전문가","방어적인 결투가","쌍수 전문가","던전 탐구자","불굴","원소 숙련",
        ...
        <section style={{ display:'grid', gridTemplateColumns:'1fr', gap:16 }}>
          <div style={{ border:'1px solid #e5e7eb', borderRadius:12, padding:16 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'120px 1fr', rowGap:8 }}>
              ...
              <div style={{ color:'#6b7280' }}>{T.skills} {classKey!=="-" ? (lang==="ko" ? L.ko.nPick(CLASS_SK_CHOICE[CLASSES[classKey].ko]?.n ?? 0) : L.en.nPick(CLASS_SK_CHOICE[CLASSES[classKey].ko]?.n ?? 0)) : ""}</div>

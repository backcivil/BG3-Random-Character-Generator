 ...
 * - 기술: 배경 2개 제외 후, 클래스 기술 풀에서 N개 랜덤
 * - “(이 중 n개 선택)” 문구 제거
 */
...
export default function App() {
  ...
  // 기술: 배경 2개 제외 후 클래스 풀에서 N개
  function rollSkills() {
    if (bg === "-") {
      setSkills([]);
      return;
    }
    const [bg1, bg2] = BG_SKILLS[bg as Exclude<Background, "-">];
      ...
      <div style={{ width: "min(1100px, 96%)", margin: "24px auto", fontFamily: "ui-sans-serif, system-ui" }}>
        ...
        <section style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16, marginTop: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <button onClick={handleRollDice} style={btn}>
              {T.rollDice}
            </button>
          </div>
        ...
        <section style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16, marginTop: 16, marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <button onClick={handleVersus} style={btn}>
              {T.vsRoll}
            </button>
          </div>
          {vsLines.length > 0 && (
